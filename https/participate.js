...
const participate = functions.https.onCall(
  ...
  async ({user, participate, note}, context) => {
    ...
    if (gameAccountUUID.error) {return { error: "no API key set" }}
    gameAccountUUID = gameAccountUUID.success

    if (!participate) {
      // user wishes to undo their participation

      let deleteDoc = await db.collection(CollectionTypes.EVENTS).doc(EVENT).collection(CollectionTypes.EVENTS__PARTICIPANTS).doc(gameAccountUUID).delete().then(() => {return true}).catch(() => {return false})
      ...
      let eventEntry = await db.collection(CollectionTypes.GAME_ACCOUNTS).doc(gameAccountUUID).collection(CollectionTypes.GAME_ACCOUNTS__EVENTS).doc(EVENT).delete().then(() => {return true}).catch(() => {return false})
    ...
    let currentValueRaw = await db.collection(CollectionTypes.EVENTS).doc(EVENT).collection(CollectionTypes.EVENTS__PARTICIPANTS).doc(gameAccountUUID).get()
    ...
    let entryResult = await db.collection(CollectionTypes.EVENTS).doc(EVENT).collection(CollectionTypes.EVENTS__PARTICIPANTS).doc(gameAccountUUID).set(entry).then(() => {return true}).catch(() => {return false})
    // inrecing teh counter for global stats
    let counter = await db.collection(CollectionTypes.EVENTS).doc(EVENT).set({ participants: admin.firestore.FieldValue.increment(1) }, { merge: true }).then(() => {return true}).catch(() => {return false})
    // adding to teh gw2 account record
    let eventEntry = await db.collection(CollectionTypes.GAME_ACCOUNTS).doc(gameAccountUUID).collection(CollectionTypes.GAME_ACCOUNTS__EVENTS).doc(EVENT).set({ entered: entryDate, gameAccountUUID: gameAccountUUID }, { merge: true }).then(() => {return true}).catch(() => {return false})
...
const participateStatus = functions.https.onCall(
  ...
  async ({user}, context) => {
    ...
    let events = await db.collection(CollectionTypes.GAME_ACCOUNTS).doc(gameAccountUUID.success).collection(CollectionTypes.GAME_ACCOUNTS__EVENTS).get()
