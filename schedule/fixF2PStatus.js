...
const fixF2PStatus = functions.pubsub.schedule("0 0 1 1 *").onRun(
  async (context) => {
    const eventDoc = db.collection(CollectionTypes.EVENTS).doc(EVENT);

    // get all users that have the f2p flag
    let allParticipants = await eventDoc.collection(CollectionTypes.EVENTS__PARTICIPANTS).where("isFreeToPlay", "==", true).get()

    // grab their api key and run it through again
    let allParticipantsData = []

    allParticipants.forEach(doc => {allParticipantsData.push(doc.data())})

    for(let i=0;i<allParticipantsData.length;i++){
      // eslint-disable-next-line no-await-in-loop
      let gameAccount = await getGw2Account(allParticipantsData[i].gameAccountUUID)
